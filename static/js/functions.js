...
function highlightNode(selected_id) {
	...
	if (selected_properties['type'] == 'deck') {
		...
		if(!show_full_flag)
			$('#additional-elements').show();      
                var translatedFromRevision = parseInt($('#'+selected_id+'-view .slideTranslatedFrom').html());                
                if (translatedFromRevision){
                    $.ajax({
                        url: './?url=ajax/getThumb&id=' + translatedFromRevision,
                        success: function(msg){
                            var data = eval('(' + msg + ')');
                            
                            var div = $("<div class='deck-menu deck-container' style='width:70%; margin-bottom:10px;'>");
                            div.html("<div class='slide' style='height:100px; width:100%;' onclick=gotoSlide(" + translatedFromRevision + ") onmouseout='hideOverlay()'>" + 
                            data.thumbnailContent + "</div>");
                            $('#additional-elements').html("Translated from:<br>");
                            $('#additional-elements').append(div).append($('#'+selected_id+'-view .slide-contributors').html());
                            overlay('right', 'max');
                        }
                    });
                }else{
...
function gotoSlide(slide){
    hideOverlay();
    window.open('./' + 'slide' + '/' + slide, '_blank');
}
...
function overlay(position, size) {
    if (!position){
        position = 'left';
    }
    if (!size){
	...
	$(".deck-container .slide")
            .mouseover(
                function() {
                    var div = $("<div>");                    
                    var html = "<div class='popover fade below in' style='display: block; width='100px''><div class='arrow right' style='left:10%;'></div><div class='inner'";
                    if(size=='max'){
                        html+= "style='width:auto;'";
                    }
                    html+= "><div class='content'><p>" + $(this).html() + "</p></div></div></div>";
                    div.html(html).attr({
                            id : $(this).attr("targetId")
                    }).appendTo($(this).parent().parent().parent());
                    div.position({
