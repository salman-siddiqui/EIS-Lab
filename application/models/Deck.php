...
//                //check if translation already exists
		...
		for ( $i=$from;$i<=$to;$i++ ) {
			if($i<count($slides)){
				...
				$slides[$i]->initiator = $slides[$i]->getInitiator();
                                $slides[$i]->wasTranslatedFromRevision = $slides[$i]->getWasTranslatedFromRevision();
	...
	public function getSlidesByRangeLite($from,$to) {
        ...
        public function getFourSlidesIndex(){
           $slides_arr=array();
            $slides = array();
            //$res = array();
            $deck_id = $this->id;
            $max_position = $this->dbGetOne('SELECT max(position) FROM deck_content WHERE deck_revision_id=:id', array('id' => $deck_id));

            $i = 1;
            while (count($slides) < 3 && $i <= $max_position){

                $item = $this->dbGetRow ( 'SELECT * FROM deck_content WHERE deck_revision_id=:id AND position=:i', array ('id' => $deck_id, 'i' => $i ) );
                if ($item['item_type'] == 'slide'){
                    $slides [] = $item;
                    $i++;
                } else {
                    $deck_id = $item['item_id'];
                    //$slides [] = $deck_id;
                    $i = 1;  
                    $max_position = $this->dbGetOne('SELECT max(position) FROM deck_content WHERE deck_revision_id=:id', array('id' => $deck_id));
                }         
            ...
            foreach ($slides as $slide){
                ...
                //$slide_obj->deck->slug_title = $slide_obj->sluggify($slide_obj->deck->title);
                //$res [] = $slide_obj;
                $slides_arr[]=array('id'=>$slide_obj->id,'deck_id'=> $slide_obj->deck->id,'position'=>$slide_obj->position,'content'=>$slide_obj->getThumbnailContent());
            }
            return $slides_arr;
}
...
//	}
	...
	public function getTree() {
		...
		$output [] = array (
                    "attr" => array (
                        'id' => 'tree-0-deck-' . $this->id . '-1' . '-node' ), 
                    'state' =>'open',
                    'data' => array (
                        "title" => $this->shortenTitle ( $this->title ), 
                        'icon' => $this->getIcon (),
                        'attr' => array (
                            'id' => 'tree-0-deck-' . $this->id . '-1',
                            'class' => 'deck-' . $this->id,
                            //'title'=>htmlspecialchars_decode($this->title).' | '.$this->getNumberOfSlides().' Slide(s) | '.$question_no.' Question(s)',
                            'title'=>htmlspecialchars_decode($this->title). ' | '.$this->getNumberOfSlides().' Slide(s)',
                            'href' => '#tree-0-deck-' . $this->id . '-1-view') ),
	...
	public function getTreeNodes() {
		...
		foreach ( $this->content as $v ) {
			...
			if (get_class ( $v ) == 'Deck'){				
				...
				$output [] = array (
                                    "attr" => array (
                                        'id' => 'tree-' . $this->id . '-deck-' . $v->id . '-' . $v->position . '-node' ), 
                                    'data' => array (
                                        "title" => $v->shortenTitle ( $v->title ), 
                                        'icon' => $v->getIcon (),
                                        'attr' => array (
                                            'id' => 'tree-' . $this->id . '-deck-' . $v->id . '-' . $v->position, 
                                            'class' =>   'deck-' . $v->id, 
                                            //'title'=>htmlspecialchars_decode($v->title).' | '.$v->getNumberOfSlides().' Slide(s) | '.$question_no.' Question(s)',
                                            'title'=>htmlspecialchars_decode($v->title).' | '.$v->getNumberOfSlides().' Slide(s)',
                                            'href' => '#tree-' . $this->id . '-deck-' . $v->id . '-' . $v->position . '-view' ) ),
                                    'children' => $childs );
			}else{
				$output [] = array (
                                    "attr" => array (
                                        'id' => 'tree-' . $this->id . '-slide-' . $v->id . '-' . $v->position .'-node' ),
                                    'data' => array (
                                        "title" => $v->shortenTitle ( $v->title ),
                                        'icon' => $v->getIcon (),
                                        'attr' => array (
                                            'id' => 'tree-' . $this->id . '-slide-' . $v->id . '-' . $v->position, 'class' => 'slide-' . $v->id,
                                            //'title'=>htmlspecialchars_decode($v->title).' | Position:'.($index_counter+1).$question_no.'| Question(s)'  ,
                                            'title'=>htmlspecialchars_decode($v->title).' | Position:'.($index_counter+1),
                                            'href' => '#tree-' . $this->id . '-slide-' . $v->id . '-' . $v->position .'-view' ) ) );
				$index_counter++;
			}
			$childs = array ();
		}
