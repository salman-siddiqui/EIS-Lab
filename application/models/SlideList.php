...
class SlideList extends Model {
	...
	public function searchMatch($keywords, $order, $page, $user_id = false, $language = false) {
		...
		// query here
		$res = $this->dbQuery( 'SELECT * FROM slide WHERE translated_from IS NULL');
		$slides = array();
		foreach($res as $index => $slide){
			$slide1 = new Slide();
			$slide1->id = $slide['id'];
			$slides[] = $slide1;
		}		
        ...
        function getAllTranslated(){
		// query here
		$res = $this->dbQuery( 'SELECT * FROM slide WHERE translated_from IS NOT NULL');
		$slides = array();
		foreach($res as $index => $slide){
			$slide1 = new Slide();
			$slide1->id = $slide['id'];
                        $revisions = $this->dbQuery('SELECT * FROM slide_revision WHERE slide=:slide_id',array('slide_id'=>$slide1->id));
