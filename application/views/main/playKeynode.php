...
<!-- first add the Addon -->
<script src="<?php echo KEYNODE_WATCHER; ?>keynode.state.js"></script>
<script src="<?php echo KEYNODE_WATCHER; ?>keynode.video.js"></script>
<!-- then add the watcher-core-file-->
<script src="<?php echo KEYNODE_WATCHER; ?>keynode.watch.js"></script>
	...
	{{each slides}}
		<div class="slide {{if $value.position==1}}{{if $value.deck.id==$data.id}}first-slide{{else}}first-sub-slide{{/if}}{{/if}}" id="tree-${$value.deck.id}-slide-${$value.id}-${$value.position}-${$value.absolute_position}-view">
			<div class="slide-content">
				...
				<div class="slide-body" style="min-height:650px;">
...
<meta name="password" content="<?php echo md5(md5($keynode_server_password)); ?>">
<link rel="http://ns.aksw.org/keynode/server" href="<?php echo KEYNODE_SERVER; ?>" />
<link rel="http://ns.aksw.org/keynode/canonical" href="<?php echo ($keynode_canonical); ?>" />
...
<script type="text/javascript">// <![CDATA[
        $(document).ready(function() {
            ...
            //add qr code
            if(!($('#qrcode').length)){
                $('#slide-area').append('<div id="qrcodediv"> <p class="qr-close-btn"> <a style="cursor:pointer;font-weight:bold;" title="Use \'Q\' shortcut to toggle the QR code">x</a>&nbsp;</p><p class="ui-widget-header">Follow this presentation on your device:<br/><a href="" id="qrlink" target="_blank"><span id="qrlinktext"></span></a> </p><div id="qrcode"></div></div>');
                var link="http://slidewiki.org/playKeynode/sid/"+sid+"/style/"+style+"/transition/"+transition+"/deck/"+deck;
                $.ajax({
                    url : './?url=ajax/getShortPlaySyncURL&sync=keynode&q='+link,
                    success : function(msg) {
                        $('#qrcode').qrcode(msg);
                        $('#qrlink').attr('href',msg);
                        $('#qrlinktext').html(' '+msg+' ');
                        $("#qrcodediv").draggable({ cancel: "span" });
                        $('#qrcodediv .qr-close-btn>a').bind('tap',function() {
                            $("#qrcodediv").hide();
                        });
                        $('#qrcodediv .qr-close-btn>a').bind('click',function() {
                            $("#qrcodediv").hide();
                        });
                        if(presenter){
                             $("#qrcodediv").hide();
