...
<header class="page-header">
	...
	<span id="deck_title"><h2><? echo _('Test for:') . $test_title . _('course'); ?>  </h2> 
        </span> 
...
<section class="deck-container deck-single">
    <article name="quests-area" id="quests-area" test_sum="0" max_sum="0" attempt="<?php echo $attempt?>">
        ...
        foreach ($questions as $question) { 
            ...
            <div class="slide" name="question-<?php echo $question->id; ?>" id="question-<?php echo $question->id; ?>-<?php echo $question->module['id']; ?>">	
                ...
                <form class="form-stacked">
                    <fieldset>
                        <legend><h3><? echo _('Question'); ?> <?php echo $i;?> of <?php echo $count;?></h3></legend>
        ...
        <div class="slide" name="question-last" id="question-last">
                <a class="btn small primary" id="countTestButton" onclick="showResults(json_obj)"><? echo _('View the results'); ?></a>
                <?php if($user['is_authorized']) : ?>
                <a class="btn small primary" id="saveTestButton" style="display: none;" onclick="saveTest()"><? echo _('Save the results'); ?></a>
   

                <?php endif; ?>
                <div id="countModeButtons" style="display:none;">
                <a class="btn small primary" id="wiki_app" onclick="showModel('wiki_app')"><? echo _('Guessing-based scoring'); ?></a> 
                <a class="btn small primary" id="continious" onclick="showModel('dich')"><? echo _('Dichotomous scoring'); ?></a>
                <a class="btn small primary" id="old_model" onclick="showModel('morgan')"><? echo _('Morgan algorythm'); ?></a>
                <a class="btn small primary" id="moodle" onclick="showModel('mtf')"><? echo _('MTF scoring'); ?></a>
                <a class="btn small primary" id="moodle" onclick="showModel('ripkey')"><? echo _('Ripkey scoring'); ?></a>
                </div>
    ...
    <footer>
        <nav>
            ...
            <form action="." method="get" class="goto-form">
                <label for="goto-slide"><? echo _('Go to question:'); ?></label>
...
<header class="page-header">
	...
	<span id="deck_title"><h1><? echo _('Test for:') . $test_title . _('course'); ?> </h1> 
            <?php if ($type!='list') :?>
            <h4><a href="./deck/<?php  
             echo $test_id . '_' . $slug_title; ?>"><? echo _('Go to the presentation'); ?></a></h4>
            <?php endif; ?>
        </span> 
...
<section class="deck-container deck-single">
    <article name="quests-area" id="quests-area" test_sum="0" max_sum="0" attempt="<?php echo $attempt?>">
        ...
        foreach ($questions as $question) { 
            ...
            ?>				
            <div class="slide" name="question-<?php echo $question->id; ?>" id="question-<?php echo $question->id; ?>-<?php echo $question->module['id']; ?>">	
                <h3 class="module_name" name="<?php echo $question->module['name']; ?>"><? echo _('Module'); ?> "<?php echo $question->module['name']; ?>"</h3>
                <form class="form-stacked">
                    <fieldset>
                    ...
                    <div class="actions">
                        <a class="btn small primary" name="showAnswersButton" onclick="showAnswers(<?php echo $question->id; ?>)"><? echo _('Show answers'); ?></a>
                        <?php if($type=='auto') :?> 
                            <a class="btn small primary" name="showSlideButton" onclick="showAutoSlide(<?php echo $question->item_id?>,<?php echo $question->module['id'] ?>,<?php echo $question->id; ?>)">
                            <? echo _('Show slide'); ?>
                            </a>
                        <?php else : ?>
                            <a class="btn small primary" name="showSlideButton" onclick="showListSlide(<?php echo $question->item_id?>,<?php echo $question->id; ?>)">
                                <? echo _('Show slide'); ?>
                            </a>
                        <?php endif; ?>
                    </div>
        ...
        <div class="slide" name="question-last" id="question-last">
                ...
                <?php endif; ?>
                <a class="btn small primary" id="tryAgainButton" onclick="tryAgain(<?php echo $test_id ?>,'<?php if ($type=='list'): echo 'user'; else: echo 'auto'; endif; ?>')">
                    <? echo _('Try it again!'); ?>
                </a>
...
<script id="module" type="text/x-jquery-tmpl">
    <div class= 'resultsDiv' name = 'moduleDiv' wiki_app=0 dich=0 mtf=0 morgan=0 ripkey=0 wiki_app=0 maxForUser=${max_for_user} id=${item_id}>
        <div style="float:left; width: 50%"><h4>${title}</h4></div><div style="text-align:right;">[<a style="cursor: pointer" onclick="collapse(${item_id})"><? echo _('Collapse'); ?></a>] [<a style="cursor: pointer" onclick="expand(${item_id})"><? echo _('Expand'); ?></a>]</div>
        <table>
            <thead>            
                <th><? echo _('Question'); ?></th>
                <th><? echo _('Difficulty'); ?></th>
                <th style='display:none'><? echo _('Not displayed'); ?></th>
                <th><? echo _('Points'); ?></th>
            </thead>
            <tbody>
