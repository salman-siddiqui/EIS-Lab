<?php
require_once (ROOT . DS . 'libraries' . DS . 'backend' . DS . 'scss.inc.php');
...
class AjaxController extends Controller {
	...
	function getShortPlaySyncURL(){
			$q=$_GET['q'];
             $namespace="sp";
              if(isset($_GET['sync'])&&$_GET['sync']=='keynode'){
                 $namespace="kn";
             }
			$u=new Url();
			$u->user_id=$this->getCurrentUserID();
			$short=$u->addget_short_url($q);
			echo BASE_PATH.$namespace."/".$short;
	}
	function getAvatarSrc(){
		...
		if($cache->bEnabled && $picture = $cache->getData('user_avatar_'.$user_id)){
			print $picture;
		}else{
			$user=new User();
			$user->createFromID($user_id);
                        //$user->picture = $user->getPicture();
						//I disabled the FB image for now
						$user->picture='';
                        if (strlen($user->picture)) 
                            $picture = file_get_contents($user->picture) ;
                        else {
                            $email=$user->getEmail();
                            $picture = file_get_contents("http://www.gravatar.com/avatar/".md5(strtolower(trim($email)))."?d=retro&amp;r=g");
			}
			if($cache->bEnabled){
				$cache->delData('user_avatar_'.$user_id);
				$cache->setData('user_avatar_'.$user_id, $picture);
			}
			print $picture;	
		}	
	}
...
//                        }
	...
	function getNewDecks(){
		...
		if($cache->bEnabled && $output=$cache->getData('new_decks')){
			...
			foreach($decks as $deck) {
                $decks_arr[]=array('id'=>$deck->id,'title'=>$deck->title,'slug_title'=>$deck->slug_title,'abstract'=>$deck->abstract,'number_of_slides'=>count( $deck->getSlidesIds() ),'slides'=>$deck->getFourSlidesIndex());
			}
			$output = array ("decks"=>$decks_arr);
			if($cache->bEnabled){
	...
	//keynode.js
	function getNotes(){
				header('Access-Control-Allow-Origin: '.KEYNODE_SERVER);
				$id = $_GET ['id'];
				$deck = new Deck ();
				$deck->createFromIDLite($id);
				$deck->content=$deck->fetchDeckContentLite ();
        ...
        function getThumb(){
            ...
            $slide->thumbnailContent = $slide->getThumbnailContent();
            echo json_encode($slide);
        }
