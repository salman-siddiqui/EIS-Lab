...
class MainController extends Controller {
	...
	function deck() {
		...
		$this->set ( 'page_description', $deck->abstract );				
                 if(defined(KEYNODE_ENABLED) && KEYNODE_ENABLED=="true") {
                    $this->set ('keynode', true);
                    
                    $pw=substr(md5($deck->id."mySalt"),-15);
                    $this->set ('keynode_server_password', $pw);
                    $canonical=BASE_PATH."playKeynode/deck/".$deck->id;
                    $this->set ('keynode_canonical', $canonical);
                    $data=array(
                      "c"=>$canonical,
                      "ns"=>array(array(
                          "url"=>KEYNODE_SERVER,
                    ...
                    $this->set ('keynode_url', KEYNODE_PRESENTER."#:!/".  base64_encode(json_encode($data)));
                } else { 
                    $this->set ('keynode', false);
                }
		if (! $style) {
	...
	function changeStyle(){
		...
		header( 'Location: style/'.$styleid.'/deck/'.$deckid) ;
	}
	function style() {
		$deckid = $_GET ['deck'];
